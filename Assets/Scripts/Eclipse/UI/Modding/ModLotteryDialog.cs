...
namespace Eclipse.UI.Modding
{
    ...
    internal sealed class ModLotteryDialog : IDisposable
    {
            ...
            Action completed, Action<ModUiCloseReason> deferred, Action<Exception> report)
        {
            ...
            var body = new ModUiNode("reward_text", ModUiKind.Text, 548, 600, text: rewardText,
            ...
            var root = new ModUiNode("lottery", ModUiKind.Column, 600, 480, gap: 8, children: new[] {
                ...
                new ModUiNode("reward", ModUiKind.Scroll, 560, 240, children: new[] { body }),
                new ModUiNode("status", ModUiKind.Text, 560, 58, text: "Your reward is saved until you claim it.",
                    style: new ModUiStyle(fontSize: 20, textAlign: "center")),
                new ModUiNode("actions", ModUiKind.Row, 560, 64, gap: 16, children: new[] {
                    new ModUiNode("claim", ModUiKind.Button, 264, 56, text: "CLAIM"),
                    new ModUiNode("later", ModUiKind.Button, 264, 56, text: "LATER")
                })
            });
            Surface = scope.Open("lottery", ModUiMount.Modal, root, id => {
                ...
                try
                {
                    if (!claim())
                    {
                        if (!Surface.IsClosed) Surface.SetText("status", "This reward is no longer available. Close and reopen the lottery.");
                        return;
                    }
                    claimed = true;
                    Surface.Close();
                ...
                {
                    if (!Surface.IsClosed) Surface.SetText("status", "The reward could not be saved. Reload your profile before continuing.");
                    report?.Invoke(error);
                }
                finally
                {
