...
namespace Newtonsoft.Json.Schema
{
	internal class JsonSchemaBuilder
	{
		...
		private JsonSchemaType? ProcessType()
		{
			...
			case JsonToken.StartArray:
			{
				...
				while (_reader.Read() && _reader.TokenType != JsonToken.EndArray)
				{
					...
					result = ((!result.HasValue) ? ((JsonSchemaType?)null) : new JsonSchemaType?(result.GetValueOrDefault() | MapType(_reader.Value.ToString())));
				}
				return result;
			}
			default:
		...
		internal static JsonSchemaType MapType(string type)
		{
			...
			if (!JsonSchemaConstants.JsonSchemaTypeMapping.TryGetValue(type, out value))
			{
