	...
	public class XmlNodeConverter : JsonConverter
	{
		...
		private void SerializeNode(JsonWriter writer, IXmlNode node, XmlNamespaceManager manager, bool writePropertyName)
		{
			...
			case XmlNodeType.Element:
			{
				...
				writer.WriteStartObject();
				for (int num = 0; num < node.Attributes.Count; num++)
				{
					SerializeNode(writer, node.Attributes[num], manager, true);
				}
			...
			do
			{
				...
				case JsonToken.PropertyName:
				{
					...
					if (reader.TokenType == JsonToken.StartArray)
					{
						...
						if (num == 1 && WriteArrayAttribute)
						{
							IXmlElement element = currentNode.ChildNodes.CastValid<IXmlElement>().Single((IXmlElement n) => n.LocalName == propertyName2);
							AddJsonArrayAttribute(element, document);
						}
					}
					else
