			...
			if (reader.TokenType == JsonToken.PropertyName)
			{
				...
				do
				{
					...
					else if (string.Equals(a, "$type", StringComparison.Ordinal))
					{
						...
						CheckedRead(reader);
						TypeNameHandling? typeNameHandling = ((member == null) ? ((TypeNameHandling?)null) : member.TypeNameHandling);
						if (((!typeNameHandling.HasValue) ? base.Serializer.TypeNameHandling : typeNameHandling.Value) != TypeNameHandling.None)
						{
							string typeName;
							string assemblyName;
