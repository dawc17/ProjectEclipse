...
namespace Newtonsoft.Json.Serialization
{
	public class DefaultContractResolver : IContractResolver
	{
		...
		protected virtual JsonProperty CreatePropertyFromConstructorParameter(JsonProperty matchingMemberProperty, ParameterInfo parameterInfo)
		{
			...
			SetPropertySettingsFromAttributes(jsonProperty, parameterInfo, parameterInfo.Name, parameterInfo.Member.DeclaringType, MemberSerialization.OptOut, out allowNonPublicAccess, out hasExplicitAttribute);
		...
		protected virtual IList<JsonProperty> CreateProperties(Type type, MemberSerialization memberSerialization)
		{
			...
			return jsonPropertyCollection.OrderBy((JsonProperty p) =>
			{
				int? order = p.Order;
		...
		private void SetPropertySettingsFromAttributes(JsonProperty property, ICustomAttributeProvider attributeProvider, string name, Type declaringType, MemberSerialization memberSerialization, out bool allowNonPublicAccess, out bool hasExplicitAttribute)
		{
			...
			property.NullValueHandling = ((attribute == null) ? ((NullValueHandling?)null) : attribute._nullValueHandling);
			property.DefaultValueHandling = ((attribute == null) ? ((DefaultValueHandling?)null) : attribute._defaultValueHandling);
			property.ReferenceLoopHandling = ((attribute == null) ? ((ReferenceLoopHandling?)null) : attribute._referenceLoopHandling);
			property.ObjectCreationHandling = ((attribute == null) ? ((ObjectCreationHandling?)null) : attribute._objectCreationHandling);
			property.TypeNameHandling = ((attribute == null) ? ((TypeNameHandling?)null) : attribute._typeNameHandling);
			property.IsReference = ((attribute == null) ? ((bool?)null) : attribute._isReference);
