...
$fixture=Join-Path $root ('Temp/LocationMotionApi-'+[Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $fixture | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ValidateLocationMotionApi.cs') -Destination (Join-Path $fixture 'Program.cs')
...
dotnet run --project (Join-Path $fixture 'Fixture.csproj') -- (Join-Path $fixture 'Mods') (Join-Path $root 'Mods/example.animated-arena') (Join-Path $root 'Mods/example.dojo-selector')
if ($LASTEXITCODE -ne 0) { throw 'Location motion API checks failed.' }
