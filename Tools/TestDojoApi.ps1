...
$fixture=Join-Path $root ('Temp/DojoApi-'+[Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $fixture | Out-Null
$marker=Join-Path $fixture 'eclipse-dojo-api-fixture.marker'
'Owned Eclipse dojo API fixture' | Set-Content -Encoding UTF8 -LiteralPath $marker
try {
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ValidateDojoApi.cs') -Destination (Join-Path $fixture 'Program.cs')
...
if ($LASTEXITCODE -ne 0) { throw 'Dojo selection checks failed.' }
    ...
    if ([System.IO.Path]::GetDirectoryName($target) -ne $fixtureRoot -or
        ...
        [System.IO.Path]::GetFileName($target) -notmatch '^DojoApi-[0-9a-f]{32}$' -or
        !(Test-Path -LiteralPath $marker -PathType Leaf) -or
        (Get-Content -Raw -LiteralPath $marker).Trim() -ne 'Owned Eclipse dojo API fixture') {
        throw "Refusing to clean an unverified dojo API fixture: $target"
    }
