...
$fixture=Join-Path $root ('Temp/StoryApi-'+[Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $fixture | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ValidateStoryApi.cs') -Destination (Join-Path $fixture 'Program.cs')
...
if($LASTEXITCODE -ne 0){throw 'Lua story checks failed.'}
