...
Copy-Item (Join-Path $root 'Mods/de128/scripts/content/combo_sphere3.lua') (Join-Path $package 'scripts/content/combo_sphere3.lua')
...
$catalog=Load-Lua 'require("content.combo_sphere3")'
Check ($catalog.Moves.Count -eq 4 -and $catalog.MoveTemplates.Count -eq 1) 'Incomplete ComboSphere3 graph.'
$doc=Project $catalog
$doc.Save((Join-Path $root 'Temp/DE128ComboSphere3.projected.xml'))
$mapping=@{
 combo_sphere3_player='ComboSphere3Player'; combo_sphere3_start='ComboSphere3Start';
 shop_magic_combo_sphere3_player='ShopMagicComboSphere3Player'; shop_magic_try_on_combo_sphere3_player='ShopMagicTryOnComboSphere3Player'
}
function Normalize-Sphere([Xml.XmlElement]$element) {
 foreach($attribute in @($element.Attributes)) {
  ...
  if($attribute.Name -eq 'Template') {$attribute.Value=(@($attribute.Value.Split('|') | Where-Object {$_ -ne 'Sphere3' -and $_ -ne 'de128:move-templates/combo_sphere3'}) -join '|')}
...
foreach($key in $mapping.Keys) {
 ...
 foreach($template in $actual.GetAttribute('Template').Split('|')) {
  if($template -eq 'de128:move-templates/combo_sphere3') {continue}
...
Check ((Fingerprint $catalog) -ceq (Fingerprint (Load-Lua 'require("content.combo_sphere3")'))) 'Sphere graph fingerprint changed on reload.'
Write-Output "PASS $script:checks combined checks: complete four-move ComboSphere3 Lua graph, template equivalence, binary identity and native attacks. No live casting/contact claim."

$attack=$doc.SelectSingleNode('//Move[@Name="de128:moves/combo_sphere3_start"]/Intervals/Interval[@Type="Attack"]')
$native=Parse-Attack $attack
Check ($native.GetReactionName(12) -ceq 'HighLong') 'Native long high-hit reaction was rewritten.'
Check ($native.IKPJJAEIOCG().Count -eq 5) 'Repeated archived attack edges were lost.'
$reactionLua='sf2.moves.register_template {id="reaction",intervals={{type="Attack",attack={edges={"Edge"},hit="HighLong"}}}}'
$reaction=Load-Lua $reactionLua
Check ((Fingerprint $reaction) -cne (Fingerprint (Load-Lua $reactionLua.Replace('HighLong','High')))) 'Reaction missing from fingerprint.'
foreach($bad in @('Highlong','highlong','Unknown')) {
 $failure=$null;try {$null=Load-Lua $reactionLua.Replace('HighLong',$bad)}catch{$failure=$_}
 Check ($null -ne $failure) ('Unsupported reaction accepted: '+$bad)
}
Write-Output "PASS $script:checks combined checks including exact HighLong reaction and repeated edges."
