...
$fixture = Join-Path $root ('Temp/FightPatches-' + [Guid]::NewGuid().ToString('N'))
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ValidateFightPatches.cs') -Destination (Join-Path $fixture 'Program.cs')
...
"@ | Set-Content -Encoding UTF8 (Join-Path $fixture 'FightPatches.csproj')
dotnet run --project (Join-Path $fixture 'FightPatches.csproj') -- (Join-Path $fixture 'Mods') $root
if ($LASTEXITCODE -ne 0) { throw 'Fight patch runtime fixture failed.' }
