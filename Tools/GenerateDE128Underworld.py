...
Reads the reviewed owner raid_stages_default.xml and historical Assets/DExml
(stages.xml templates, list.xml, localizations) once, offline, and writes typed Lua tables under
...
def lua(value, indent=0, compact=False):
    pad = "    " * indent
    if value is None:
        return "nil"
    ...
    if isinstance(value, list):
        if not value:
            return "{}"
        simple = compact or all(not isinstance(v, (dict, list)) for v in value)
        if simple:
            return "{ " + ", ".join(lua(v, compact=True) for v in value) + " }"
    ...
    if isinstance(value, dict):
        ...
        # Keep each warrior, perk, template and dialogue card on one readable
        # row. Containers (zones, battles, fights) retain their visible structure.
        compact = compact or bool(set(value) & {'template', 'perk', 'text'}) or (
            'name' in value and 'attributes' in value and 'fights' not in value)
        parts = [key(k) + " = " + lua(v, indent + 1, compact) for k, v in items]
        one = "{ " + ", ".join(parts) + " }"
        if compact or (len(one) + len(pad) <= 118 and "\n" not in one):
            return one
        return "{\n" + "".join(pad + "    " + p + ",\n" for p in parts) + pad + "}"
...
class Generator:
    def __init__(self):
        source = OWNER_RAID.read_bytes()
        if hashlib.sha256(source).hexdigest() != OWNER_RAID_SHA256:
            raise ValueError("Owner raid source changed; review its semantic differences before regenerating")
        self.raid = ET.fromstring(source)
        ...
        for index, zone in enumerate(self.raid.iter("Zone")):
            ...
            for battle in zone.findall("Battle"):
                ...
                for fight in battle.findall("Fight"):
                    ...
                    fights.append({
                        "name": fight.get("Name"), "power": int(fight.get("Power", "0")) or None,
                        "rounds": rounds if rounds > 0 else 1, "round_time": int(fight.get("RoundTime")),
                        "replays": int(fight.get("Replays", "0")) or None,
            ...
            by_name = {battle['name']: battle for battle in battles}
            for battle in battles:
                if battle['power_mode'] != 'power':
                    continue
                normal = by_name.get(battle['name'].removesuffix('_HARDMODE'))
                if normal is None:
                    continue
                originals = {fight['name']: fight for fight in normal['fights']}
                for index, fight in enumerate(battle['fights']):
                    base = originals.get(fight['name'])
                    # A nil cannot clear a field in a Lua row. Keep a full row
                    # whenever the archive removes an inherited optional value.
                    if base is None or any(value is not None and fight.get(key) is None for key, value in base.items()):
                        continue
                    delta = {'name': fight['name'], 'inherit': normal['name']}
                    delta.update({key: value for key, value in fight.items() if value != base.get(key)})
                    if len(delta) < len(fight):
                        battle['fights'][index] = delta
    ...
    def render(self):
        ...
        header = "-- GENERATED by Tools/GenerateDE128Underworld.py from reviewed DE sources; do not edit by hand.\n"
        data = header + "return " + lua({"templates": templates, "zones": zones}) + "\n"
        story_text = header + "return " + lua(story) + "\n"
        text = header + key_module(values['eng'], 'uw.').replace('return text\n', '''function text.key(key)
...
''')
        ...
        translations = {}
        for language, words in values.items():
            entries = {'uw.' + key: value for key, value in words.items()}
            for tier, key in enumerate(ZONE_IDS, 1):
                if key in words:
                    entries['zones/underworld_tier_' + str(tier)] = words[key]
            translations[language] = entries
        return data, text, story_text, notes, translations
...
def main() -> int:
    ...
    data, text, story, notes, translations = Generator().render()
    outputs = {OUT / "underworld_data.lua": data, OUT / "underworld_text.lua": text,
               OUT / "underworld_story_data.lua": story}
    outputs.update(section_outputs('GENERATED Underworld (Tools/GenerateDE128Underworld.py)', translations, ''))
