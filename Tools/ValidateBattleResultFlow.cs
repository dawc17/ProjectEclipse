...
public class ListSF {
 ...
 public QuestParameters HAOHNNFLOGK;public static Roster CCDKHLAMKKO()=>Profile;public static ListSF GetInstance()=>Current;
 public static FightList CHMCKGCDGCM(FightIDS id)=>Program.Fight;public static int IDMJOMOMDOJ()=>0;public static void KNCECPAINLI(Battle b){}public static Battle MKHAAGMJOPG(FightIDS id)=>Program.Fight.Battle;
...
public class Fight {public static Fight GetCurrentFight()=>new Fight();public void BCFBHJOLGNL(FightResult r){Program.Trace.Add("presentation");Program.Present?.Invoke();}}
...
public static class Program {
 ...
 public static void Main(){
  ...
  Reset();Lottery=true;Run();Check(delivered==1&&!Trace.Contains("quest")&&ListSF.GetInstance().HAOHNNFLOGK!=null,"deferred lottery observation");
  Check(SourceEvaluations==0,"Lottery detection re-evaluated reward definitions");
  Reset();DefinedLottery=true;Run();Check(Trace.Contains("quest")&&ListSF.GetInstance().HAOHNNFLOGK==null,"Lottery from another reward scope suppressed quest progression");
  Reset();Lottery=true;Run(GameOverTypes.GAME_OVER_LOSS);Check(Trace.Contains("quest")&&ListSF.GetInstance().HAOHNNFLOGK==null,"Loss deferred a lottery claim");
  Reset();Lottery=true;Run(GameOverTypes.GAME_OVER_SURRENDER);Check(Trace.Contains("quest")&&ListSF.GetInstance().HAOHNNFLOGK==null,"Surrender deferred a lottery claim");
