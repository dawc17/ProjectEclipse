...
internal static class DE128FoundationTests
{
    ...
    private static void CheckDE(ModContentCatalog catalog)
    {
        ...
        Check(catalog.Moves.Count == 25 && catalog.MoveItemLockExtensions.Count == 10 && catalog.MoveCombatPatches.Count == 5,
        ...
        Check(catalog.ItemAvailabilityPolicies.Count() == 46 &&
        ...
        Check(catalog.Weapons.Count(item => !item.IsCore) == 11 && catalog.ShopListings.Count == 21,
        ...
        Check(catalog.Rewards.Count == 1 && catalog.TryGetReward(
            ...
            reward.Choices.Count == 0 && reward.Gems == 0 && catalog.ItemDefaultEnchantments.Count == 21 && !catalog.ItemDefaultEnchantments.Any(value => value.Item == Sword),
...
");
    ...
    private static void CheckCombatEquipment(string source, string fixture, string repository, ModDescriptor mod, ModContentCatalog catalog)
    {
        ...
        foreach (var patch in catalog.ItemCombatSubtypes)
        {
            ...
            bool ownedFamily = patch.Subtype == "ChineseSwords" && catalog.Moves.Count == 25 &&
    ...
    private static void Run(string source, string fixture, string repository)
    {
        ...
        foreach (string module in new[] { "timers", "equipment", "combat_equipment", "chinese_swords", "chinese_swords_data", "restored_weapons", "restored_equipment", "sphere1", "sphere2", "sphere3", "shared_moves", "shop", "rewards", "combat_perks", "progression" })
        {
