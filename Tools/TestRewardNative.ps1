...
foreach($name in @('Reward','RewardStruct','RewardPrize','RewardChoice','RewardLottery','Rewardable','LogMessage')) {
...
$program.Replace('/* BUILDERS */',($methods -join "`n")).Replace('/* ITEM CONSTRUCTOR */',$constructor.Value).Replace('/* ITEM SELECTION */',$selectItem.Value) | Set-Content -LiteralPath (Join-Path $fixture 'Program.cs') -Encoding UTF8
