...
$fixture = Join-Path $root ('Temp/StoryDialog-' + [Guid]::NewGuid().ToString('N'))
...
Copy-Item (Join-Path $PSScriptRoot 'ValidateStoryDialog.cs') (Join-Path $fixture 'Program.cs')
...
if ($LASTEXITCODE -ne 0) { throw 'Story dialog fixture failed.' }
