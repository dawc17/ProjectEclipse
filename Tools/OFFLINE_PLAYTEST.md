...
Use Unity **2022.3.62f3**. The project now packages gameplay XML and uses local
...
It packs all non-`.meta` files from `Assets/vanillaXml` (including relative
quest/location includes) into `Assets/Resources/SF2Content/gameplay.bytes`. Missing required files
...
The editor reads `Assets/vanillaXml`. Players extract the bundled resource
...
The packaged vanilla 2.41.9 source is versioned by archive hash; size changes with
the imported data set. Allow room and time for extraction on first launch. Later
launches reuse the completed copy.
