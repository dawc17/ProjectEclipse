...
$fixture = Join-Path $root ('Temp/SenseiRaidCharge-' + [Guid]::NewGuid().ToString('N'))
$content = Join-Path $fixture 'Mods/fixture.raid/scripts/content'
New-Item -ItemType Directory -Force $content | Out-Null
foreach ($name in @('sensei_raid_charge_state')) {
    Copy-Item (Join-Path $root "Mods/de128/scripts/content/$name.lua") $content
}
Copy-Item (Join-Path $PSScriptRoot 'ValidateSenseiRaidCharge.cs') (Join-Path $fixture 'Program.cs')
...
"@ | Set-Content (Join-Path $fixture 'Raid.csproj')
dotnet run --project (Join-Path $fixture 'Raid.csproj') -- (Join-Path $fixture 'Mods') $root
if ($LASTEXITCODE -ne 0) { throw 'Sensei RaidCharge fixture failed.' }
