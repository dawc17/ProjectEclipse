...
$fixture=Join-Path $root ('Temp/SceneNavigationLua-'+[Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $fixture | Out-Null
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'ValidateSceneNavigationLua.cs') -Destination (Join-Path $fixture 'Program.cs')
...
if($LASTEXITCODE -ne 0){throw 'Lua navigation checks failed.'}
