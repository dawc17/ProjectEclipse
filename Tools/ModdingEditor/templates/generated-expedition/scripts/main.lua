...
local function alias(key) return "example.generated-expedition:localization/" .. key end
...
sf2.modes.register {
    ...
    on_prepare = function(request, event)
        local challenge, duration = false, 60
        sf2.ui.open {
            id = "expedition", mount = "menu",
            root = { id = "root", kind = "column", width = 460, height = 330, gap = 12,
                children = {
                    { id = "title", kind = "text", width = 460, height = 60,
                      text = "Expedition: encounter " .. event.step, style = { font_size = 28 } },
                    { id = "challenge", kind = "toggle", width = 460, height = 48, text = "Stronger opponent" },
                    { id = "time", kind = "text", width = 460, height = 32, text = "Round time: 60 seconds" },
                    { id = "duration", kind = "slider", width = 460, height = 48, value = 0.5 },
                    { id = "begin", kind = "button", width = 460, height = 60, text = "BEGIN ENCOUNTER" },
                },
            },
            on_change = function(view, id, value)
                if id == "challenge" and type(value) == "boolean" then challenge = value
                elseif id == "duration" and type(value) == "number" then
                    duration = math.floor(30 + value * 60)
                    sf2.ui.set_text(view, "time", "Round time: " .. duration .. " seconds")
                end
            end,
            on_click = function(view, id)
                if id ~= "begin" or not sf2.modes.is_pending(request) then return end
                local opponent = fighters[sf2.random.integer("route_rng", 1, #fighters)]
                sf2.modes.resolve(request, {
                    warriors = { opponent }, level = event.step + (challenge and 3 or 0),
                    rounds = 1, round_time = duration,
                })
                sf2.ui.close(view)
            end,
            on_close = function() sf2.modes.cancel(request) end,
        }
        -- No result yet: the request stays pending until the player chooses.
    end,
}
