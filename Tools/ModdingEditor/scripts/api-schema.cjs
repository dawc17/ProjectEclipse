...
type('FightPatch',{target:'string','description?':'string','rounds?':'integer','round_time?':'integer','location?':'string','music?':'string','warriors?':H('Warrior')+'[]','rules?':H('Rule')+'[]','append_rules?':H('Rule')+'[]'});reg('fights.patch','FightPatch',null,'content.patch');
...
type('ProfileItemSnapshot',{present:'boolean',owned:'boolean',count:'integer',equipped:'boolean','upgrade?':'integer'});fn('profile.level',{},'integer','profile.read');fn('profile.item',{item:H('Item')},E('ProfileItemSnapshot'),'profile.read');
type('StorySubscription',{'private __eclipseStorySubscription':'true'});type('StoryEvent',{kind:'"purchase"|"enchantment"|"level_up"|"scene_enter"','item?':'string','recipe?':'string','previous_level?':'integer','level?':'integer','scene?':'"map"|"shop"|"profile"|"dojo"|"fight"'});
fn('story.on',{event:'"purchase"|"enchantment"|"level_up"|"scene_enter"',callback:'fun(event: Eclipse.StoryEvent)'},E('StorySubscription'),'story.events');
