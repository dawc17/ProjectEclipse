...
const moveCondition=`${E('MovePerkCondition')}|${E('MoveNamedCondition')}|${E('MoveConditionGroup')}|${E('MoveCharacterCondition')}|${E('MoveKeysCondition')}|${E('MoveStageCondition')}|${E('MoveScreenCondition')}|${E('MoveModCondition')}`;
...
type('MoveAttack',{edges:'string[]','damage?':'number','damage_type?':enumOf('UnarmedDamage','WeaponDamage','RangedDamage','MagicDamage'),'damage_terms?':E('MoveDamageTerm')+'[]','hit?':enumOf('High','Middle','Low','Spinning','HighHeavy','MiddleShortPlus'),'id?':'integer','impulse?':E('MoveImpulse')});
...
type('MoveProjectile',{name:'string',core_skeleton:'string',copy_parent_type:enumOf('Weapon','Ranged','Magic'),'core_start_animation?':'string','start_move?':H('Move')});
type('MoveBulletChange',{type:enumOf('MagicBullet','RaidChargeBullet'),value:'integer'});
type('MoveScheduledAction',{type:enumOf('random_sound','try_on_end','effect','stop_effect','stop_follow_effect','create_projectile','add_bullets','delete_actor'),'frame?':'integer','event?':enumOf('RoundStage','KeyPressed','KeyReleased','RoundStart','RoundEnd','Hit','Strike','WallHit','AnimationStart','AnimationEnd','IntervalStart','IntervalEnd','EveryFrame','Birth','ModExpires'),'core_sounds?':'string[]','effect?':E('MoveEffect'),'effect_name?':'string','projectile?':E('MoveProjectile'),'bullets?':E('MoveBulletChange'),'player?':enumOf('Me','Enemy','Parent','Child','EnemyChild')});
...
type('MoveTemplateDefinition',move);type('MoveDefinition',{...move,animation:H('Binary'),'transitions?':E('MoveTransition')+'[]','actions?':E('MoveScheduledAction')+'[]','profile?':E('MoveProfile'),'tactic_distance?':E('MoveTacticDistance'),'no_wall_repulsion?':'boolean','no_interpolation_frames?':'boolean'});reg('moves.register_template','MoveTemplateDefinition','MoveTemplate');reg('moves.register','MoveDefinition','Move');
...
type('MoveHitPatch',{expected:enumOf('High','Middle','Low','Spinning','HighHeavy','MiddleShortPlus'),value:enumOf('High','Middle','Low','Spinning','HighHeavy','MiddleShortPlus')});
