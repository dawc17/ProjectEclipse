...
type('FightPatch',{target:'string','description?':'string','rounds?':'integer','round_time?':'integer','location?':'string','music?':[`${H('Audio')}|string`,'Audio handle for mod-owned music, or an installed native track name.'],'warriors?':H('Warrior')+'[]','reward_drops?':E('RewardDropPatch')+'[]','rules?':H('Rule')+'[]','append_rules?':H('Rule')+'[]'});reg('fights.patch','FightPatch',null,'content.patch');
...
type('MoveAnimationPatch',{expected:'string',value:H('Binary')});
...
fn('visuals.rim_light',{definition:visual('RimLightDefinition',{'offset?':['number','0-12 pixels, default 2.5.'],'alpha?':['number','0-1, default 0.85.'],'lighten?':['number','0-1, default 0.35.'],'ink?':['number','0-1: rim turns to ink_color while casting magic, default 0.'],'ink_color?':['string','#RRGGBB or #RRGGBBAA ink colour.']})},'nil','presentation.visuals');
