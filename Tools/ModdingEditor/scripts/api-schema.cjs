...
const actionNames=['show_battle','toggle_battle','map_focus','fight','current_fight','eclipse','update_eclipse_battles','give_item','set_variable','show_map_button','dialog','story'];
const actionUnion=actionNames.map(n=>E('Action_'+n)).join('|');
type('DialogButton',{text:'string','color?':'string',actions:`(${actionUnion})[]`});
for (const [name,fields] of Object.entries({show_battle:{battle:H('Battle'),'locked?':'boolean'},toggle_battle:{battle:H('Battle'),'visible?':'boolean'},map_focus:{battle:H('Battle')},fight:{fight:H('Fight')},current_fight:{},eclipse:{'enabled?':'boolean'},update_eclipse_battles:{},give_item:{item:H('Item')},set_variable:{name:'string',value:'string'},show_map_button:{id:'string',image:H('Sprite')+'|string',x:'number',y:'number','anchor_min_x?':'number','anchor_max_x?':'number','show_type?':enumOf('both','story','raid')},dialog:{'title?':'string','image?':'string',lines:`(string|${E('DialogLine')})[]`,'button?':E('DialogButton')},story:{lines:`(string|${E('DialogLine')})[]`}})) type('Action_'+name,{type:JSON.stringify(name),...fields});
...
type('StorySubscription',{'private __eclipseStorySubscription':'true'});type('StoryEvent',{kind:'"purchase"|"enchantment"|"level_up"|"scene_enter"|"map_button"|"item_acquired"|"battle_result"','fight?':'string','outcome?':'"win"|"loss"|"surrender"|"raid_timeout"|"raid_round_timeout"','eclipse?':'boolean','equipment?':E('BattleEquipmentSnapshot')+'[]','item?':'string','recipe?':'string','previous_count?':'integer','count?':'integer','previous_level?':'integer','level?':'integer','scene?':'"map"|"shop"|"profile"|"dojo"|"fight"','button?':'string'});
...
fn('story.on',{event:'"purchase"|"enchantment"|"level_up"|"scene_enter"|"map_button"|"item_acquired"|"battle_result"',callback:'fun(event: Eclipse.StoryEvent)'},E('StorySubscription'),'story.events');
...
type('MoveIntervalStartPatch',{name:enumOf('Uninterrupt','SelfUninterrupt','Unstable'),expected:'integer',value:'integer'});
