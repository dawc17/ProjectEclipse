...
static class Program {
 ...
 static void Main(string[] args){
  ...
  foreach(string scenario in new[]{"good","denied","forged","unknown","error","loop","nohost","example","level","scene"}){
  ...
  on_click=function(view,id) sf2.ui.close(view) end}
   ...
   using(var context=new MoonSharpScriptRuntime(views.Add,null,null,scenario=="nohost"?null:bus).CreateContext(mod,
    ...
    if(!expected){
     ...
     Check(errors.Count==(scenario=="good"||scenario=="example"||scenario=="level"||scenario=="scene"?0:1),"callback errors "+scenario);
     if(scenario=="scene"){
   ...
   Check(!bus.HasSubscribers(ModStoryEventKind.Purchase)&&!bus.HasSubscribers(ModStoryEventKind.LevelUp)&&!bus.HasSubscribers(ModStoryEventKind.SceneEnter),"context disposal retained subscriptions");
  }
