...
and `example.enchantment` for the API 0.2 template-derived perk/enchantment slice.
...
## Perks and enchantments API (0.2)

API 0.2 exposes perks as template-derived combat behavior and enchantments as forge
availability for that behavior. It intentionally does not expose the recovered
`perks.xml` trigger/action language directly. External perks derive from a registered
perk template, usually a qualified vanilla `core:perks/...` definition, and override
only the template's `<Set>` parameters plus presentation fields.

`sf2.perks.get(reference)` resolves a registered perk and returns an opaque perk handle.
Unqualified references resolve in the calling mod's namespace. Qualified references are
allowed only for `core` or declared dependencies. For example, the exact vanilla weapon
Lifesteal variant used by the Medium forge recipe is available as:

```lua
local lifesteal_template = sf2.perks.get(
    "core:perks/PERK_ITEM_SPECIAL_LIFESTEAL_WEAPON"
)
```

Create a derived perk with `sf2.perks.register`:

```lua
local lifesteal = sf2.perks.register {
    id = "lifesteal",
    template = lifesteal_template,
...
The resulting definition ID is `<mod-id>:perks/lifesteal`. `display_name` and
`description` must be localization handles owned by the registering mod. `icon` is an
optional sprite handle. When omitted, the perk keeps the template's canonical logical
image name. That matters because vanilla deliberately resolves the same perk image through
different shop and in-fight sprite paths. Supply `icon` only when intentionally replacing
the inherited presentation. `parameters` is optional; keys map to template `<Set>`
attributes and values may be strings, finite numbers, or booleans. Formula strings are
preserved for the recovered perk/function evaluator rather than evaluated by Lua
registration. Forge strength (`Aspect`) is generated by the selected vanilla recipe tier,
so normal forge enchantments should not set `Aspect` here.

`sf2.enchantments.register` makes a newly registered perk available through one of the
recovered forge recipe tiers:
...
Recipe constants are `SIMPLE`, `MEDIUM`, and `COMPLEX`. Equipment constants are
`WEAPON`, `ARMOR`, `HELM`, `RANGED`, and `MAGIC`; `item_types` must be a non-empty dense
array and cannot contain duplicates. API 0.2 requires the enchantment's perk to have been
registered by the same mod transaction. Simple and medium recipes accept single perks;
complex recipes require a combo perk. The returned enchantment handle is opaque and the
definition ID is `<mod-id>:enchantments/<id>`.

Registration remains atomic with the rest of `content.register`: if the entrypoint fails
or the transaction does not commit, its localizations, perks, enchantments, items, and shop
entries do not leak into the frozen catalog. Committed perk/enchantment definitions are
included in the deterministic save `contentHash`. Published perk IDs should therefore be
treated as stable save-facing identity once the perk is written into an item's
`<Enchantments>` data.

`Mods/example.enchantment` is the smallest tracked example. It derives from the canonical
vanilla lifesteal perk, reuses the existing core lifesteal icon, and exposes the result as a
medium weapon enchantment without copying or reimplementing any combat triggers.
