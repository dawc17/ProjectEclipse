...
local function locks(player, shop)
    local result = {
        { type = "item", item_type = player and "Magic" or "Weapon", item_subtype = "Sphere3" },
...
local function actor() return { type = "actor_name", name = "Sphere3" } end
...
local function effect(stage, frame, scale, time_scale, position)
    return { type = "effect", frame = frame, effect = {
        name = "Sphere3" .. stage, core_sequence = "mgc_magic_big_sphere_" .. string.lower(stage),
        scale = scale, time_scale = time_scale, position = position, follow = false,
    } }
end
local function spawn(start_move)
    return { type = "create_projectile", frame = 5, projectile = {
        name = "Sphere3", core_skeleton = "SkeletonMagic", copy_parent_type = "Magic", start_move = start_move,
    } }
end
...
register("sphere3_start", {
    ...
    events = { "birth" }, actions = { effect("Start", 2, 0.75, 4, point("Nodes", "Magic-Node2_1", "Me", 0, -150)) },
...
register("sphere3_middle", {
    ...
    align = { axes = { "X", "Z" }, pivot = point("Nodes", "Magic-Node2_1"), position = point("Nodes", "Magic-Node2_1", "Me") },
    events = { "animation_end" }, intervals = { { type = "Attack", start = 22, attack = {
        ...
        damage = 0.6, damage_terms = { { type = "MagicDamage" }, { type = "UnarmedDamage", shift = -25 } },
    ...
    } } }, actions = { effect("End", 10, 2, 2, point("Nodes", "Magic-Node2_1", "Me", 0, -150)),
        { type = "delete_actor", player = "Me", event = "AnimationEnd" }, sound(9, "snd_bigsphere_end") },
    conditions = { current("StanceShop", "Parent", true), actor() }, locks = locks(false),
})
for _, row in ipairs({ { "shop_magic_sphere3_player", "ShopPeacefulStart", "PeacefulStart" },
    ...
    register(row[1], {
        ...
        locks = locks(true, true), actions = {
            effect("Start", 22, 0.75, 4, point("Wall", "Back", "Me", 415, 160)),
            effect("End", 27, 2, 1.5, point("Wall", "Back", "Me", 400, 150)), sound(21, "snd_bigsphere_end"),
