...
local function locks(player, shop)
    local result = {
        { type = "item", item_type = player and "Magic" or "Weapon", item_subtype = "Sphere2" },
...
local function actor() return { type = "actor_name", name = "Sphere2" } end
...
local function effect(stage, when, x, y, follow, scale, time_scale, looped)
    return { type = "effect", frame = type(when) == "number" and when or nil,
        event = type(when) == "string" and when or nil, effect = {
            name = "Energyball" .. stage, core_sequence = (stage == "End" and "mgc_magic_mid_sphere1_end" or "mgc_magic_mid_sphere_" .. string.lower(stage)),
...
local function spawn(start_move)
    return { type = "create_projectile", frame = 2, projectile = {
        name = "Sphere2", core_skeleton = "SkeletonMagic", copy_parent_type = "Magic", start_move = start_move,
    } }
end
...
local function strike_actions()
    return { sound("Strike", "snd_midsphere_end"),
        { type = "delete_actor", player = "Me", event = "Strike" },
        effect("End", "Strike", 5, 0, false, 1.5, 1, false) }
end
...
local start_actions = { effect("Start", 3, 25, 0, true, 1, 1.1, false),
    effect("Middle", "AnimationEnd", 5, 0, true, 1, 1, true),
    sound(2, "snd_midsphere_start"), sound(12, "snd_midsphere_middle") }
...
register("sphere2_middle", {
    ...
    events = { "animation_end" }, conditions = { { type = "any", conditions = {
        current(name("sphere2_start")), current(name("sphere2_middle")),
    } }, actor() }, intervals = { attack(nil, true) }, actions = strike_actions(), locks = locks(false),
})
register("sphere2_wall", {
    animation = animation.middle, templates = { family }, core_templates = { "MagicMissileFly", "MagicMissile" },
    mid_frames = 2, first_frame = 1, priority = 500, no_magic_recharge = true,
    align = { axes = { "X", "Y", "Z" }, pivot = point("Nodes", "Magic-Node2_1"), position = point("Nodes", "Magic-Node2_1", "Me") },
    events = { "every_frame" }, conditions = { current(name("sphere2_middle")),
...
register("shop_magic_sphere2", {
    ...
    actions = { effect("Start", 3, 25, 0, true, 1, 1.1, false),
        effect("Middle", "AnimationEnd", 5, 10, true, 1, 1, true), sound(2, "snd_midsphere_start"),
        sound(12, "snd_midsphere_middle"), { type = "delete_actor", player = "Me", event = "AnimationEnd" } },
...
register("shop_magic_try_on_sphere2_start", {
    ...
    mid_frames = 2, first_frame = 2, priority = 1, no_wall_repulsion = true, no_interpolation_frames = true, no_magic_recharge = true,
    actions = { effect("Start", 3, 25, 0, true, 1, 1.1, false), effect("Middle", "AnimationEnd", 5, 10, true, 1, 1, true),
        sound(2, "snd_midsphere_start"), sound(12, "snd_midsphere_middle") },
...
register("shop_magic_try_on_sphere2_end", {
    ...
    mid_frames = 2, first_frame = 1, priority = 500, no_magic_recharge = true, velocity = { x = 30 },
    align = { axes = { "X", "Y", "Z" }, pivot = point("Nodes", "Magic-Node2_1"), position = point("Nodes", "Magic-Node2_1", "Me") },
    events = { "animation_end" }, conditions = { current(name("shop_magic_try_on_sphere2_start")), actor() },
    actions = { { type = "stop_effect", effect_name = "EnergyballMiddle", frame = 2 },
        effect("End", 2, 5, 10, false, 1.5, 1, false), sound(2, "snd_midsphere_end") }, locks = locks(false),
})
return moves
