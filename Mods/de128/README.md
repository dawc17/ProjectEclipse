...
halo, and uses the shop/profile scroll frame, scrollbar and elastic scrolling. The paintbrush button sits in the bottom-left corner of the story map,
beside the chapter title. Enable the mod, Apply & Restart, then enter the dojo
and click the paintbrush to choose a dojo. Version `0.40.0` restores four
...
attack, shows the hand effect, and follows the archived 800-frame normal or
700-frame Power Mode opening and recast cooldown.
...
Generated warrior, perk and dialogue data uses compact rows; Power Mode fights
store differences from their normal twin. Translations live in
`localizations/<language>.toml`, including the Sensei story and Titan rewards.
The Lua text modules only resolve keys from those files.
